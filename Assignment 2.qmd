...
#| fig-cap: "BMI og inntekt – nedprøvd scatter og trend per kjønn (1.–99. persentil)."
library(dplyr)
library(ggplot2)
library(scales)
#| label: fig-bmi-inntekt-ryddig
#| fig-cap: "BMI og inntekt
#| fig-width: 10
#| fig-height: 6


# Bruk datasettet du allerede har laget
base <- heights_bmi |>
  rename(inntekt = income, kjonn = sex, BMI = bmi) |>
  mutate(kjonn = forcats::fct_relevel(kjonn, "Menn", "Kvinner"))

# Grenser for y-akse (1.–99. persentil, men behold hele datasettet til beregning)
lims <- quantile(base$inntekt, c(.01, .99), na.rm = TRUE)
...
ggplot(df_plot, aes(BMI, inntekt, colour = kjonn)) +
  geom_point(alpha = 0.10, size = 0.8) +                           # lette punkter
  geom_smooth(data = base, method = "lm", se = FALSE, linewidth = 1.2) +  # trend på hele datasettet
